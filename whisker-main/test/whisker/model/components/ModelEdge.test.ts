...
describe('Model edges', () => {
    ...
    test("Reset does not clear conditions on ModelEdge", () => {
        const edge = new ProgramModelEdge(id, label, graphID, from, to, -1, -1);
        const condition = new BackgroundChange(label, {id, negated: false, args: ["test"]});
    ...
    test("Program model edge", () => {
        const effect = new BackgroundChange(label, {id, negated: false, args: ["test"]});
        const edge = new ProgramModelEdge(id, label, graphID, from, to, -1, -1);
        const condition = new BackgroundChange(label, {id, negated: false, args: ["test"]});
    ...
    describe('checkConditions()', () => {
        test("checkConditions() returns conditions when no step happened", () => {
            ...
            edge.addCondition(new BackgroundChange(label, {id, negated: false, args: ["test"]}));
            edge.addCondition(new Key(label, {id, negated: false, args: ["a"]}));
            edge.addCondition(new SpriteTouching(label, {id, negated: false, args: ["apple", "bowl"]}));
    ...
    describe("checkConditionsOnEvent()", () => {
        ...
        test("checkConditionsOnEvent() returns conditions when event string not contained", () => {
            ...
            const edge = new ProgramModelEdge(id, label, graphID, from, to, -1, -1);
            edge.addCondition(new BackgroundChange(label, {id, negated: false, args: ["newBackground"]}));
            edge.addCondition(new Key(label, {id, negated: false, args: ["d"]}));
            edge.addCondition(new SpriteTouching(label, {id, negated: false, args: ["banana", "bowl"]}));
        ...
        test("checkConditionsOnEvent() returns conditions when true is condition and edge has no effect", () => {
            ...
            edge.addCondition(new BackgroundChange(label, {id, negated: false, args: ["newBackground"]}));
            edge.addCondition(new Key(label, {id, negated: false, args: ["a"]}));
            edge.addCondition(new Expr(label, {id, negated: false, args: ["true"]}));
            edge.addEffect(new SpriteTouching(label, {id, negated: false, args: ["apple", "bowl"]}));
