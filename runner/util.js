...
const logger = {
    ...
    debug: (...args) => console.log(chalk.white.bgBlue.bold(' DEBUG: '), ...args, '\n')
};
...
const cli = {
    start: () => {
        commander
            .option('-u, --whiskerURL <URL>', 'URL of the Whisker instance to run the tests', '../dist/index.html')
            ...
            .option('-c, --isConsoleForwarded', 'If the browser\'s console output should be forwarded');

        commander.parse(process.argv);

        const {whiskerURL, testPath, scratchPath, frequency, isHeadless, numberOfTabs, isConsoleForwarded} = commander;
